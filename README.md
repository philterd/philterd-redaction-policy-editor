...
- **Schema-Driven**: The entire form is generated from the official Phileas redaction policy
  [JSON Schema](https://github.com/philterd/phileas/tree/main/policy-schema). Generated policies are
...
- **Multiple Schema Versions**: Select which policy schema version to author. The editor bundles one
  folder per version under `src/main/resources/schemas/<version>/`; adding a new version is just a
  matter of dropping in its schema file — no code changes required.
