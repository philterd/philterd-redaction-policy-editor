...
### Can I test a policy without entering real data?
Yes. The **Load Sample Text** menu on the Test Policy input fills it with one of four synthetic documents: a clinical note, a financial record, a legal filing, and a support email. Each contains fabricated names, identifiers, dates, and contact details that exercise the common filters. None of it belongs to a real person.

### Does testing require a running Philter instance?
No. The editor bundles the Phileas redaction engine and runs it in process, so testing works with no Philter deployment, no API key, and no network access. One consequence is that filters backed by an AI model, such as PhEye, cannot be tested in the editor: those need a running PhEye service. Rule-based filters, meaning regex, dictionary, and validator driven detection, test normally.
